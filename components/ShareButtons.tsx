...
/** SNSシェアボタン（X / LINE / Facebook）。ブランドカラーのアイコン付き。リンクのみでサーバーコンポーネントで完結。 */
export function ShareButtons({ slug, title }: { slug: string; title: string }) {
  ...
  return (
    <div className="not-prose mt-12 flex flex-wrap items-center gap-x-3 gap-y-3 border-t border-[var(--hairline)] pt-7">
      ...
      {shares.map((s) => (
        <a
          ...
          className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-medium text-white shadow-sm transition-transform hover:-translate-y-0.5 ${s.className}`}
        >
