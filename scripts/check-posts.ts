...
const VALID_CATEGORIES = ["tsurai", "shokushu", "template", "kaisha", "flow", "real"];
const AFFILIATE_CATEGORIES = ["kaisha", "flow", "real"];
...
for (const file of files) {
  ...
  const isAffiliateCat = AFFILIATE_CATEGORIES.includes(fm.category as string);
  if (content.includes("AffiliateSlot") && !isPR) {
    err(file, "AffiliateSlot を使っているのに isPR: true がありません");
  }
  if (isAffiliateCat && !isPR) {
    warn(file, "アフィ対象カテゴリですが isPR が立っていません（PR明示を確認）");
  }
  if (isAffiliateCat && !(fm.sources as unknown[])?.length) {
    warn(file, "出典(sources)が空です（数値の記述には出典を推奨）");
  }
}
